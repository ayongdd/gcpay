...
class BetRecordAmountPage
    extends BaseStatefulWidget<BetRecordAmountController> {
  BetRecordAmountPage({
    Key? key,
  }) : super(key: key);
  ...
  @override
  String titleString() {
    return "BettingRecordAmountDetail".tr;
  }
}
...
class BetRecordAmountController extends BaseController {
  RxString rxTransferType = 'usdt'.obs;
  final GCPayApi _gcpayApi = Get.find<GCPayApi>();
  RxList<BetRecordTotalEntity> betList = <BetRecordTotalEntity>[].obs;
  @override
  void loadNet() {}
  ...
  Future<void> getBetRecordInfo(String strTransferType) async {
    try {
      ...
      var response;// = await _gcpayApi.getTronInRecordBetAmount(data);
...
class BetRecordAmountBinding extends Bindings {
  @override
  void dependencies() {
    Get.lazyPut(() => BetRecordAmountController());
  }
}
